...
function browse_auctions(&$sqlr, &$sqlc)
{
    ...
    if((isset($_GET['search_value']) && isset($_GET['search_by'])) || (isset($_GET['search_class'])) || (isset($_GET['search_quality'])) )
    {
        ...
        $query_1 = $sqlc->query("SELECT count(*) FROM `".$characters_db[$realm_id]['name']."`.`auctionhouse` LEFT JOIN `".$characters_db[$realm_id]['name']."`.`characters` ON `auctionhouse`.`itemowner`=`characters`.`guid` LEFT JOIN `".$characters_db[$realm_id]['name']."`.`characters` c2 ON `c2`.`guid`=`auctionhouse`.`buyguid` LEFT JOIN `".$characters_db[$realm_id]['name']."`.`item_instance` ON `auctionhouse`.`itemguid`=`item_instance`.`guid` LEFT JOIN `".$world_db[$realm_id]['name']."`.`item_template` ON `item_instance`.`itemEntry`=`item_template`.`entry` WHERE 1 = 1 $search_filter $order_side");
    }
    else
    {
    ...
    $result = $sqlc->query("SELECT BINARY `characters`.`name` AS `seller`, `item_instance`.`itemEntry` AS `itemid`, `item_template`.`name` AS `itemname`, `auctionhouse`.`buyoutprice` AS `buyout`, TIMESTAMPDIFF(SECOND, NOW(), FROM_UNIXTIME(`auctionhouse`.`time`)), BINARY `c2`.`name` AS `encherisseur`, `auctionhouse`.`lastbid`, `auctionhouse`.`startbid`, `item_instance`.`count` AS qty, `characters`.`race` AS seller_race, `c2`.`race` AS buyer_race FROM `".$characters_db[$realm_id]['name']."`.`auctionhouse` LEFT JOIN `".$characters_db[$realm_id]['name']."`.`characters` ON `auctionhouse`.`itemowner`=`characters`.`guid` LEFT JOIN `".$characters_db[$realm_id]['name']."`.`characters` c2 ON `c2`.`guid`=`auctionhouse`.`buyguid` LEFT JOIN `".$characters_db[$realm_id]['name']."`.`item_instance` ON `auctionhouse`.`itemguid`=`item_instance`.`guid` LEFT JOIN `".$world_db[$realm_id]['name']."`.`item_template` ON `item_instance`.`itemEntry`=`item_template`.`entry` WHERE 1 = 1 $search_filter $order_side ORDER BY `auctionhouse`.`$order_by` $order_dir LIMIT $start, $itemperpage");
